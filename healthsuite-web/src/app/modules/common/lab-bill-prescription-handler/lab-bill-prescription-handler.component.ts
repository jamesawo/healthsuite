...
import { ClerkLabRequestPayload } from '@app/shared/_payload/clerking/clerk-request.payload';
...
@Component({
    selector: 'app-lab-bill-prescription-handler',
    templateUrl: './lab-bill-prescription-handler.component.html',
    styleUrls: ['./lab-bill-prescription-handler.component.css'],
})
export class LabBillPrescriptionHandlerComponent implements OnInit, OnDestroy, OnChanges {
    @Input('props')
    public props: {
    ...
    @Output('selected')
    public selected: EventEmitter<ClerkLabRequestPayload> = new EventEmitter<ClerkLabRequestPayload>(
        null
    );
    @Output('isSearching')
    ...
    onViewSelectedItem(item: ClerkLabRequestPayload, i: number) {
        if (item) {
